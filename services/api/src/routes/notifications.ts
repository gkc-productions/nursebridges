...
export async function notificationRoutes(app: FastifyInstance) {
  app.get("/notifications", async (req, reply) => {
    ...
    if (error) {
      if (isMissingNotificationsTable(error)) return reply.send({ notifications: [] });
  ...
  app.post("/notifications/:id/read", async (req, reply) => {
    ...
    if (error) {
      if (isMissingNotificationsTable(error)) return reply.code(404).send({ error: "Notification not found" });
      return reply.code(400).send({ error: "Unable to update notification" });
    }
    if (!data) return reply.code(404).send({ error: "Notification not found" });
  ...
  app.post("/notifications/read-all", async (req, reply) => {
    ...
    if (error) {
      if (isMissingNotificationsTable(error)) return reply.send({ ok: true });
