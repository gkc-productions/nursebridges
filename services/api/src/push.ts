...
export function looksLikeExpoPushToken(token: string) {
...
export async function sendExpoPushToUser(input: {
  ...
  if (!supabaseAdmin || !input.userId) return;
  ...
  if (error) {
    if (!isMissingPushTokensTable(error)) {
      console.error("push destination lookup failed", { user_id: input.userId, code: (error as any).code });
    }
    return;
  }
