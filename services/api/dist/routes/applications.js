...
export async function applicationRoutes(app) {
    ...
    app.post("/v1/applications/:applicationId/decide", async (req, reply) => {
        ...
        if (body.decision === "accept") {
            ...
            if (!(await isApprovedNurse(appRow.nurse_user_id))) {
        ...
        if (body.decision === "accept") {
            ...
            if (rejectError)
                return reply.code(400).send({ error: "Unable to decide application" });
            const { error: jobUpdateError } = await markJobAssigned(appRow.job_id, appRow.nurse_user_id);
            if (jobUpdateError)
                return reply.code(400).send({ error: "Unable to decide application" });
            ...
            await createNotifications([
                {
                    userId: appRow.nurse_user_id,
                    type: "job_assigned",
                    title: "Job assigned",
                    body: `${jobTitle} has been assigned to you.`,
                    entityType: "job",
                    entityId: appRow.job_id
                },
                ...((rejectedApplications ?? [])
                    .filter((row) => row.status !== "rejected")
                    .map((row) => ({
                    userId: row.nurse_user_id,
