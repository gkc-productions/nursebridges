...
function spec({ includeRpc = false, missingColumn = null } = {}) {
  ...
  return {
    openapi: "3.0.0",
    components: { schemas },
    paths: includeRpc ? { "/rpc/finalize_applied_assignment_rpc": { post: {} } } : {}
  };
}
...
function runChecker(url, args = []) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, ["scripts/ops/check-assignment-rpc-contract.mjs", ...args], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        SUPABASE_URL: url,
...
describe("assignment RPC contract checker", () => {
  it("allows a missing RPC in prerequisite mode", async () => {
    await withServer(spec(), async (url) => {
      const result = await runChecker(url);

      assert.equal(result.code, 0);
      assert.match(result.stdout, /RPC status: \/rpc\/finalize_applied_assignment_rpc is not exposed yet/);
  ...
  it("requires the RPC in strict post-apply mode", async () => {
    await withServer(spec(), async (url) => {
      ...
      assert.match(result.stderr, /missing \/rpc\/finalize_applied_assignment_rpc/);
    });
  });

  it("fails when a required assignment column is missing", async () => {
    await withServer(spec({ includeRpc: true, missingColumn: "jobs.assigned_nurse_user_id" }), async (url) => {
      const result = await runChecker(url, ["--expect-rpc"]);

      assert.equal(result.code, 1);
      assert.match(result.stderr, /jobs\.assigned_nurse_user_id/);
    });
  });
});
