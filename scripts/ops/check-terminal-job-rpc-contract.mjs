...
const RPC_NAME = "finalize_terminal_job_rpc";
const RPC_PATH = `/rpc/${RPC_NAME}`;

function usage() {
  console.log(`Terminal job RPC contract check
...
if (missingColumns.length > 0) {
  fail(`Supabase REST schema metadata is missing required terminal job columns: ${missingColumns.sort().join(", ")}`);
}
...
console.log("Result: terminal job RPC prerequisites satisfy the read-only Supabase metadata contract.");
