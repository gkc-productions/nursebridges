...
Assignment stale-write hardening now claims the open job before mutating application statuses in API admin assignment, patient application acceptance, and admin web assignment paths. Stale claim failures now map to the shared `conflict` error category, reducing ambiguous failure handling. The local API now defaults admin assignment and patient application acceptance to `finalize_applied_assignment_rpc`; live Supabase function apply/exposure and deployment are still required before broader beta.

Terminal cancel/complete paths now use the shared error contract for stale guarded status writes. API cancel/complete returns 409 conflict and admin terminal actions throw `statusCode: 409` with `category: "conflict"` when the final status claim updates no job row; tests also prove notifications and audit rows are not written after stale terminal writes. The local API now defaults patient/nurse/admin terminal actions to `finalize_terminal_job_rpc`; live Supabase function apply/exposure and deployment are still required before broader beta.
...
An assignment RPC design package exists in `docs/architecture/sql/assignment-finalize-rpc.draft.sql` and `docs/ops/assignment-rpc-rollout-plan.md`. The committed migration `supabase/migrations/20260911015426_add_atomic_job_finalizers.sql` creates the approved assignment and terminal finalizer functions, but it has not been applied to live Supabase and does not authorize production deployment by itself.

The API has an RPC finalizer contract in `services/api/src/jobAssignmentCommand.ts`. It calls `finalize_applied_assignment_rpc` with job, application, nurse, and actor context, maps database/RPC errors back to the shared workflow categories, and reports when the RPC path owns the durable audit side effects.

API admin assignment and patient application acceptance now call assignment through an injectable finalizer. Runtime registration defaults to the RPC-backed finalizer; focused route tests keep the guarded multi-write fallback covered for local/internal proof.
...
Terminal cancel/complete now has its own database-atomicity package: `docs/architecture/sql/terminal-job-finalize-rpc.draft.sql`, `docs/ops/terminal-job-rpc-rollout-plan.md`, and the committed migration `supabase/migrations/20260911015426_add_atomic_job_finalizers.sql`. This targets the known partial-write risk where cancellation rejects applications before the final job status claim. It has not been applied to live Supabase and does not authorize production deployment by itself.
...
`scripts/ops/check-rpc-sql-safety.mjs` now guards the assignment and terminal SQL drafts plus the committed migration for server-only execution posture, job-row locks, service-role grants, notification/audit durability, and care-request notification privacy before any owner-approved Supabase apply.
...
The API has a terminal RPC finalizer contract in `services/api/src/jobTerminalCommand.ts`. It calls `finalize_terminal_job_rpc` with job, actor, expected-status, and next-status context, and maps database/RPC errors back to the shared workflow categories. Runtime registration defaults to the RPC-backed finalizer while focused route tests keep the guarded multi-write fallback covered.

Local verification for the atomic finalizer work passed: API typecheck, API route/helper tests, operation readiness guards, and full root `pnpm verify`. VM deployment, live Supabase function exposure checks, and production smoke remain separate approval-gated steps; the broader 131-file staged package VM resume checklist remains in `docs/release/pending-vm-verification.md`.
