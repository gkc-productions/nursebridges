...
export async function GET(request: NextRequest) {
  ...
  if (quotes.error || earnings.error) return adminJson(request, { error: "Unable to load finance ledger" }, { status: 400 });
  return adminJson(request, { ledger_enabled: ledgerEnabled, payment_processing_enabled: false, quotes: quotes.data ?? [], earnings: earnings.data ?? [] });
}
export async function POST(request: NextRequest) {
  ...
  if (!jobId || !Number.isInteger(patientTotalCents) || patientTotalCents < 0 || !Number.isInteger(includedMinutes) || includedMinutes < 30 || !Number.isInteger(guaranteedCents) || guaranteedCents < 0) {
  ...
  const { data: quote, error } = await supabaseAdmin.from("care_quotes").insert({
    ...
    overtime_increment_cents: Number(body.overtime_increment_cents ?? 0), status: "draft", created_by_user_id: auth.user.id
  }).select("*").single();
