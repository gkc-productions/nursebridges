-- Qualify application column references in the assignment finalizer.
-- PL/pgSQL output parameters share the function namespace, so unqualified
-- application.job_id references can become ambiguous with the returned job_id.
...
begin
  select public.jobs.id, public.jobs.status, public.jobs.title
    into v_job
    from public.jobs
  ...
  select public.applications.id,
         public.applications.job_id,
         public.applications.nurse_user_id,
         public.applications.status
    into v_application
    from public.applications
   where public.applications.id = p_selected_application_id
     and public.applications.job_id = p_job_id
   for update;
  ...
  if not exists (
    select 1
     ...
     where public.nurse_profiles.nurse_id = p_selected_nurse_user_id
       and public.nurse_profiles.verification_status = 'approved'
  ) then
    raise exception using errcode = '42501', message = 'nurse_verification_required';
  end if;

  select coalesce(array_agg(public.applications.nurse_user_id), array[]::uuid[])
    into v_rejected_nurse_user_ids
    from public.applications
   where public.applications.job_id = p_job_id
     and public.applications.status = 'applied'
     and public.applications.nurse_user_id <> p_selected_nurse_user_id;

  update public.jobs
     set status = 'assigned',
         assigned_nurse_user_id = p_selected_nurse_user_id
   where public.jobs.id = p_job_id
     and public.jobs.status = 'open';
